...
class ShuttleFivePDFController extends Controller
{
    /**
     * Print PDF for Shuttle 5 Form A
     */
    public function printFormA($id)
    {
        ...
        $data = [
            'formA' => $formA,
            'shuttle' => $shuttle,
            'title' => 'Borang 5A - Maklumat Kilang',
            'print_date' => now()->format('d/m/Y H:i:s')
        ];

        $pdf = PDF::loadView('pdf.shuttle5.form-a', $data);
        $pdf->setPaper('A4', 'portrait');
        
        return $pdf->stream("Borang_5A_{$shuttle->nama_kilang}_{$formA->tahun}.pdf");
    }

    /**
     * Print PDF for Shuttle 5 Form B
     */
    public function printFormB($id)
    {
        ...
        $data = [
            ...
            'title' => 'Borang 5B - Maklumat Guna Tenaga',
            'print_date' => now()->format('d/m/Y H:i:s')
        ];

        $pdf = PDF::loadView('pdf.shuttle5.form-b', $data);
        $pdf->setPaper('A4', 'portrait');
        
        return $pdf->stream("Borang_5B_{$shuttle->nama_kilang}_{$formB->tahun}_S{$formB->suku_tahun}.pdf");
    }

    /**
     * Print PDF for Shuttle 5 Form C
     */
    public function printFormC($id)
    {
        ...
        $data = [
            ...
            'title' => 'Borang 5C - Kemasukan Bahan Mentah',
            'print_date' => now()->format('d/m/Y H:i:s')
        ];
        ...
        $pdf = PDF::loadView('pdf.shuttle5.form-c', $data);
        $pdf->setPaper('A4', 'landscape');
        
        return $pdf->stream("Borang_5C_{$shuttle->nama_kilang}_{$formC->tahun}_{$bulan_nama[$formC->bulan]}.pdf");
    }

    /**
     * Print PDF for Shuttle 5 Form D
     */
    public function printFormD($id)
    {
        $formD = Form5D::findOrFail($id);
        ...
        $data = [
            'formD' => $formD,
            'shuttle' => $shuttle,
            'title' => 'Borang 5D - Pengeluaran dan Jualan',
            'print_date' => now()->format('d/m/Y H:i:s')
        ];
        ...
        $pdf = PDF::loadView('pdf.shuttle5.form-d', $data);
        $pdf->setPaper('A4', 'landscape');
        
        return $pdf->stream("Borang_5D_{$shuttle->nama_kilang}_{$formD->tahun}_{$bulan_nama[$formD->bulan]}.pdf");
    }

    /**
     * Print PDF for Shuttle 5 Form E
     */
    public function printFormE($id)
    {
        $formE = Form5E::findOrFail($id);
        ...
        $data = [
            'formE' => $formE,
            'shuttle' => $shuttle,
            'title' => 'Borang 5E - Maklumat Eksport',
            'print_date' => now()->format('d/m/Y H:i:s')
        ];
        ...
        $pdf = PDF::loadView('pdf.shuttle5.form-e', $data);
        $pdf->setPaper('A4', 'portrait');
        
        return $pdf->stream("Borang_5E_{$shuttle->nama_kilang}_{$formE->tahun}_{$bulan_nama[$formE->bulan]}.pdf");
    }
}
