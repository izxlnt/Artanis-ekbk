...
class FormCKayuKKS extends Component
{
    ...
    public function render()
    {
        ...
        $kembali = route('user.shuttle-5-formC.KKB', $this->bulan_id);
        ...
        return view('livewire.shuttle-five.form-c-kayu-k-k-s', compact('returnArr'));
    }
    ...
    public function store()
    {
        ...
        if ($this->sebelumnya) {
            return redirect()->route('user.shuttle-5-formC.KKB', $this->bulan_id);
        }
    ...
    public function calcJumlahKayuMasuk($keySpecies, $keyKumpulanKayu, $singkatan)
    {
        ...
        foreach ($this->species as $species_key => $data) {
            $kayu_masuk = ($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->kayu_masuk[$species_key] ?? 0) : 0;
    ...
    public function calcJumlahBakiStok($keySpecies, $keyKumpulanKayu, $singkatan)
    {
        $jumlah = 0;
        foreach ($this->species as $keySpecies => $data) {
            $baki_stok = ($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->baki_stok[$keySpecies] ?? 0) : 0;
    ...
    {
        $this->jumlah_stok_kayu_balak[$keySpecies] = (float)($this->baki_stok[$keySpecies] ?? 0) + (float)($this->kayu_masuk[$keySpecies] ?? 0);
    }
    ...
    public function calcTotalStokKayuBalak($keyKumpulanKayu, $singkatan)
    {
        $jumlah = 0;
        foreach ($this->species as $keySpecies => $data) {
            $jumlah += (float)(($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->jumlah_stok_kayu_balak[$keySpecies] ?? 0) : 0);
        }
    ...
    {
        (float)$this->baki_stok_kehadapan[$keySpecies] = (float)($this->jumlah_stok_kayu_balak[$keySpecies] ?? 0) - (float)($this->proses_masuk[$keySpecies] ?? 0);
    }
    ...
    public function calcTotalKemasukanKayuBalakJentera($keySpecies, $keyKumpulanKayu, $singkatan)
    {
        $jumlah = 0;
        foreach ($this->species as $keySpecies => $data) {
            $jumlah += (($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->proses_masuk[$keySpecies] ?? 0) : 0);
        }
    ...
    public function calcTotalPengeluaranKayuDaripadaJentera($keySpecies, $keyKumpulanKayu, $singkatan)
    {
        $jumlah = 0;
        foreach ($this->species as $keySpecies => $data) {
            $jumlah += (float)(($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->proses_keluar[$keySpecies] ?? 0) : 0);
        }
    ...
    public function calcTotalStokKayuDibawaBulanHadapan($keySpecies, $keyKumpulanKayu, $singkatan)
    {
        $jumlah = 0;
        foreach ($this->species as $keySpecies => $data) {
            $jumlah += (float)(($data->kumpulan_kayu->singkatan == $singkatan) ? ($this->baki_stok_kehadapan[$keySpecies] ?? 0) : 0);
        }
