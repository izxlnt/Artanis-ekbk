...
class FormB extends Component
{
    ...
    public $pekerja_wargabumi_lelaki, $pekerja_wargabumi_perempuan, $pekerja_bukan_wargabumi_lelaki, $pekerja_bukan_wargabumi_perempuan, $pekerja_asing_lelaki, $pekerja_asing_perempuan,
      ...
      protected $rules = [
        'gaji_lelaki.0' => 'required',
        'gaji_perempuan.0' => 'required',
    ];

    // public function updated()
    // {
    //     // dd($propertyName);
    //     // $id = substr($propertyName, strpos($propertyName, ".") + 1); // get id from
    ...
    //     $this->validate([
    //         'gaji_lelaki.0' => 'numeric|min:' . $this->min_gaji[0] . '|max:' . $this->max_gaji[0],
    //         'gaji_perempuan.0' => 'numeric|min:' . $this->min_gaji[0] . '|max:' . $this->max_gaji[0],

    //         'gaji_lelaki.1' => 'numeric|min:' . $this->min_gaji[1] . '|max:' . $this->max_gaji[1],
    //         'gaji_perempuan.1' => 'numeric|min:' . $this->min_gaji[1] . '|max:' . $this->max_gaji[1],

    //         'gaji_lelaki.2' => 'numeric|min:' . $this->min_gaji[2] . '|max:' . $this->max_gaji[2],
    //         'gaji_perempuan.2' => 'numeric|min:' . $this->min_gaji[2] . '|max:' . $this->max_gaji[2],

    //         'gaji_lelaki.3' => 'numeric|min:' . $this->min_gaji[3] . '|max:' . $this->max_gaji[3],
    //         'gaji_perempuan.3' => 'numeric|min:' . $this->min_gaji[3] . '|max:' . $this->max_gaji[3],

    //         'gaji_lelaki.4' => 'numeric|min:' . $this->min_gaji[4] . '|max:' . $this->max_gaji[4],
    //         'gaji_perempuan.4' => 'numeric|min:' . $this->min_gaji[4] . '|max:' . $this->max_gaji[4],

    //         'gaji_lelaki.5' => 'numeric|min:' . $this->min_gaji[5] . '|max:' . $this->max_gaji[5],
    //         'gaji_perempuan.5' => 'numeric|min:' . $this->min_gaji[5] . '|max:' . $this->max_gaji[5],

    //         'gaji_lelaki.6' => 'numeric|min:' . $this->min_gaji[6] . '|max:' . $this->max_gaji[6],
    //         'gaji_perempuan.6' => 'numeric|min:' . $this->min_gaji[6] . '|max:' . $this->max_gaji[6],

    //         'gaji_lelaki.7' => 'numeric|min:' . $this->min_gaji[7] . '|max:' . $this->max_gaji[7],
    //         'gaji_perempuan.7' => 'numeric|min:' . $this->min_gaji[7] . '|max:' . $this->max_gaji[7],
    //     ]);
    ...
    public function calcJumlahGaji($key) //(13)+(14)= (15)
    {
        ...
        $this->total_gaji[$key] = (float)$lelaki + (float)$perempuan;
    }
    ...
    public function calcTotalAllBayaranGajiPerPekerjaLelaki($key)   //jumlah total bayaran gaji per pekerja lelaki
    {
        ...
        foreach ($kategori_pekerja as $key => $data) {
            ...
            $this->jumlah_gaji_lelaki += (float)$lelaki;
        }
    }
    ...
    public function calcTotalAllBayaranGajiPerPekerjaPerempuan($key)   //jumlah total bayaran gaji per pekerja perempuan
    {
        ...
        foreach ($kategori_pekerja as $key => $data) {
            ...
            $this->jumlah_gaji_perempuan += (float)$perempuan;
        }
    }
    ...
    public function calcTotalAllBayaranGajiLelakiPerempuan($key)   //TOTAL SEMUA GAJI PER LELAKI+PEREMPUAN
    {
        ...
        $this->jumlah_lelaki_perempuan = 0;
        foreach ($kategori_pekerja as $key => $data) {
            $jumlah_lelaki_perempuan = $this->gaji_lelaki_perempuan[$key] ?? 0;
            $this->jumlah_lelaki_perempuan += (float)$jumlah_lelaki_perempuan;
        }
    }
    ...
    public function calcJumlahGajiUpahSebulanLelaki($key)   //(14)= (08) * (11)
    {
        ...
        $this->total_gaji_lelaki[$key] += (float)$total_gaji_lelaki;
    }
    ...
    public function calcJumlahGajiUpahSebulanPerempuan($key)   //(14)= (08) * (11)
    {
        ...
        $this->total_gaji_perempuan[$key] += (float)$total_gaji_perempuan;
    }
    ...
    public function calcTotalAllBayaranGajiPerPekerjaLelakiPerempuan($key)   //jumlah total bayaran gaji per pekerja lelaki + perempuan
    {
        ...
        $perempuan = $this->gaji_perempuan[$key] ?? 0;
        $lelaki = $this->gaji_lelaki[$key] ?? 0;
        $this->gaji_lelaki_perempuan[$key] += (float)$lelaki + (float)$perempuan;
    }
    ...
    public function calcTotalAllBayaranGajiLelaki($key)   //jumlah total bayaran gaji pekerja lelaki
    {
        ...
        foreach ($kategori_pekerja as $key => $data) {
            $lelaki = $this->total_gaji_lelaki[$key] ?? 0;
            $this->jumlah_total_lelaki += (float)$lelaki;
        }
    }
    ...
    public function calcTotalAllBayaranGajiPerempuan($key)   //jumlah total bayaran gaji pekerja perempuan
    {
        ...
        foreach ($kategori_pekerja as $key => $data) {
            $perempuan = $this->total_gaji_perempuan[$key] ?? 0;
            $this->jumlah_total_perempuan += (float)$perempuan;
        }
    }
    ...
    public function calcTotalAllBayaranGaji($key)   //jumlah total bayaran gaji pekerja lelaki + perempuan
    {
        ...
        $this->jumlah_total_gaji = 0;
        foreach ($kategori_pekerja as $key => $data) {
            $total = $this->total_gaji[$key] ?? 0;
            $this->jumlah_total_gaji += (float)$total;
        }
    }
