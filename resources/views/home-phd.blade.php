            ...
            <div class="col-12">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        {{-- SECTION 2: Shows count of forms with 'Sedang Diproses' status for current year (dynamically loaded via AJAX) --}}
                        <div class="card-header bg-info"
                            style="text-align:center; background-color: #f3ce8f !important; font-size: 130%; font-weight: bold;">
                            <h4 class="text-white m-b-0" style="background-color: #f3ce8f "><b>JUMLAH BORANG MENUNGGU PENGESAHAN PHD ({{ date('Y') }})</b></h4>
                        </div>
                    </div>
                </div>
                <br>
                    ...
                    <div class="col-lg-4 col-md-6 mb-3">
                        <div class="card bg-info card-hover" style="border-radius:25px">
                            <div class="card-body" style="box-shadow:0 4px 8px 0 rgba(0,0,0,0.2);background-color:#ee8dcd;border-radius:25px;text-align:center;">
                                ...
                                <div class="mt-2" id="phd_detail_s3" style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px"></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 mb-3">
                        <div class="card bg-info card-hover" style="border-radius:25px">
                            <div class="card-body" style="box-shadow:0 4px 8px 0 rgba(0,0,0,0.2);background-color:#f0e10dbd;border-radius:25px;text-align:center;">
                                ...
                                <div class="mt-2" id="phd_detail_s4" style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px"></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 mb-3">
                        <div class="card bg-info card-hover" style="border-radius:25px">
                            <div class="card-body" style="box-shadow:0 4px 8px 0 rgba(0,0,0,0.2);background-color:#6df173;border-radius:25px;text-align:center;">
                                ...
                                <div class="mt-2" id="phd_detail_s5" style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <script>
                (function(){
                    var detailUrl = "{{ route('ajax.count.phd.detail') }}";
                    var formLinks = {
                        3: {
                            A: "{{ route('phd.senarai-tugasan-3A', date('Y')) }}",
                            B: "{{ route('phd.senarai-tugasan-3B', date('Y')) }}",
                            C: "{{ route('phd.senarai-tugasan-3C', date('Y')) }}",
                            D: "{{ route('phd.senarai-tugasan-3D', date('Y')) }}"
                        },
                        4: {
                            A: "{{ route('phd.senarai-tugasan-4A', date('Y')) }}",
                            B: "{{ route('phd.senarai-tugasan-4B', date('Y')) }}",
                            C: "{{ route('phd.senarai-tugasan-4C', date('Y')) }}",
                            D: "{{ route('phd.senarai-tugasan-4D', date('Y')) }}",
                            E: "{{ route('phd.senarai-tugasan-4E', date('Y')) }}"
                        },
                        5: {
                            A: "{{ route('phd.senarai-tugasan-5A', date('Y')) }}",
                            B: "{{ route('phd.senarai-tugasan-5B', date('Y')) }}",
                            C: "{{ route('phd.senarai-tugasan-5C', date('Y')) }}",
                            D: "{{ route('phd.senarai-tugasan-5D', date('Y')) }}",
                            E: "{{ route('phd.senarai-tugasan-5E', date('Y')) }}"
                        }
                    };
                    var colors = {A:'#555',B:'#c9559e',C:'#8a7e00',D:'#0e7012',E:'#0a5da8'};
                    [3,4,5].forEach(function(s){
                        $.get(detailUrl, {shuttle_type: s}, function(data){
                            var container = $('#phd_detail_s'+s);
                            container.empty();
