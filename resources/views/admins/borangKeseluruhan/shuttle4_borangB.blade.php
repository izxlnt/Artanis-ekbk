                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <br><br>
                            <div>
                                <h4 class="text-center">SENARAI PENUH MAKLUMAT BORANG - BORANG 4B</h4>
...
foreach ([1 => 3, 2 => 6, 3 => 9, 4 => 12] as $q => $bulan) {
    ...
    if ($d && $d->status === 'Dihantar ke IPJPSM' && $b && $b->status === 'Dihantar ke IPJPSM' && $b->borang_b == 2) { $isPending = true; break; }
                                            ...
                                            <tr class="{{ $isPending ? 'table-warning' : '' }}">
                                                ...
                                                <td>
                                                    ...
                                                    @include('partials.cell-borang-quarterly', ['data' => $data, 'current_batch' => $cb, 'buffer' => $buffer, 'viewRoute' => 'ipjpsm.shuttle-4-view-formB', 'batchField' => 'borang_b', 'from' => 'keseluruhan', 'suku_tahun' => 1])
                                                </td>

                                                <td>
                                                    @php $data = $formBIndex[$shuttle->id][2] ?? null; $cb = $data ? ($batchIndex[$shuttle->id][6] ?? null) : null; @endphp
                                                    @include('partials.cell-borang-quarterly', ['data' => $data, 'current_batch' => $cb, 'buffer' => $buffer, 'viewRoute' => 'ipjpsm.shuttle-4-view-formB', 'batchField' => 'borang_b', 'from' => 'keseluruhan', 'suku_tahun' => 2])
                                                </td>

                                                <td>
                                                    @php $data = $formBIndex[$shuttle->id][3] ?? null; $cb = $data ? ($batchIndex[$shuttle->id][9] ?? null) : null; @endphp
                                                    @include('partials.cell-borang-quarterly', ['data' => $data, 'current_batch' => $cb, 'buffer' => $buffer, 'viewRoute' => 'ipjpsm.shuttle-4-view-formB', 'batchField' => 'borang_b', 'from' => 'keseluruhan', 'suku_tahun' => 3])
                                                </td>

                                                <td>
                                                    @php $data = $formBIndex[$shuttle->id][4] ?? null; $cb = $data ? ($batchIndex[$shuttle->id][12] ?? null) : null; @endphp
                                                    @include('partials.cell-borang-quarterly', ['data' => $data, 'current_batch' => $cb, 'buffer' => $buffer, 'viewRoute' => 'ipjpsm.shuttle-4-view-formB', 'batchField' => 'borang_b', 'from' => 'keseluruhan', 'suku_tahun' => 4])
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
    ...
    <script>
        function changePage() {
            ...
            window.location.href = "<?php echo URL::to('/admin/borang-keseluruhan/shuttle-4/borang-B/" + year +"'); ?>";
