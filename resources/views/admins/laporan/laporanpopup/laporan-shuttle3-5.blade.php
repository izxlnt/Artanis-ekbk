        ...
        <div class="page-breadcrumb" style="padding: 0px">
            <div class="pb-2 row">
                <div class="col-5 align-self-center">
                    ...
                    @if ($title == '5. Top 10 Pengeluar Kayu Gergaji di Kilang Papan')
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['5', $tahun, ($file_type = 'excel'), ($spesies = 'no_species')]) }}"
                            class="btn btn-success">Excel</a>
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['5', $tahun, ($file_type = 'pdf'), ($spesies = 'no_species')]) }}"
                            class="btn btn-danger">PDF</a>
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['5', $tahun, ($file_type = 'print'), ($spesies = 'no_species')]) }}"
                            class="btn btn-warning">Print</a>
                    @elseif($title == '6. Top 10 Kilang Papan Dalam Penggunaan Spesies Kayu Balak')
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['6', $tahun, ($file_type = 'excel'), $spesies]) }}"
                            class="btn btn-success">Excel</a>
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['6', $tahun, ($file_type = 'pdf'), $spesies]) }}"
                            class="btn btn-danger">PDF</a>
                        <a href="{{ route('laporan_shuttle_3_5.excel', ['6', $tahun, ($file_type = 'print'), ($spesies )]) }}"
                            class="btn btn-warning">Print</a>
                    @endif
                </div>
                                    ...
                                    <tr>
                                        @foreach ($columns as $data)
                                            @if ($data == 'Jumlah Pengeluaran Kayu Gergaji')
                                                <th class="text-center" rowspan="1">{{ $data }}</th>
                                            @else
                                ...
                                <tbody>
                                            ...
                                            <td class="text-right">
                                                @foreach ($datas_formc as $data)
                                                    @if ($data->shuttle_id == $kilang->shuttle->id)
                                                        {{ number_format(round($data->jumlah_pengeluaran) ?? 0) }}
                                                    @endif
                                                @endforeach
                                            </td>
                                    ...
                                    @if($kilang->jumlah_pengeluaran > 0)
                                        <tr class="text-center">
                                            ...
                                            <td class="text-left">{{ $kilang->daerah->daerah_hutan ?? '' }}</td>
                                            <td class="text-left">{{ $kilang->negeri->negeri ?? '' }}</td>
                                            <td class="text-left">
