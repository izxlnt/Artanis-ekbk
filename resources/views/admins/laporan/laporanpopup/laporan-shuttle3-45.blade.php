        ...
        <div class="page-breadcrumb" style="padding: 0px">
            <div class="pb-2 row">
                <div class="col-5 align-self-center">
                    ...
                    <a href="{{ route('laporan_shuttle_3_45.excel', ['45', $tahun, ($file_type = 'excel')]) }}"
                        class="btn btn-success">Excel</a>
                    <a href="{{ route('laporan_shuttle_3_45.excel', ['45', $tahun, ($file_type = 'pdf')]) }}"
                        class="btn btn-danger">PDF</a>
                    <a href="{{ route('laporan_shuttle_3_45.excel', ['45', $tahun, ($file_type = 'print')]) }}"
                                    ...
                                    @foreach ($datas as $pembeli => $data_pembeli)
                                        ...
                                        <tr class="text-right">
                                            ...
                                            @foreach ($data_pembeli as $negeri => $data)
                                                <td>{{ number_format(round($data[0]->jumlah_jualan)) }}</td>
                                                @php
                                                    $jumlah_row += round($data[0]->jumlah_jualan);
                                                    $jumlah_negeri[$negeri] += round($data[0]->jumlah_jualan);
                                                    $jumlah_keseluruhan += round($data[0]->jumlah_jualan);
                                                @endphp
                                            @endforeach

                                            <td>{{ number_format(round($jumlah_row)) }}</td>
                                        </tr>
                                    @endforeach
