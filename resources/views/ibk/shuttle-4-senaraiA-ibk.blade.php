...
@section('content')
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td>
                                                @if ($list && $list->status == 'Sedang Diproses')
                                                    <a href="{{ route('pengguna.shuttle-3-view-formA', $list->id) }}"><img
                                                            src="{{ asset('circle_check_yellow.png') }}" height='30px'
                                                            alt="" style="color: green; font-size: 20pt;"
                                                            data-toggle="tooltip" data-placement="bottom"
                                                            title="Borang telah dihantar"></i></a>
                                                @elseif ($list && $list->status == 'Lulus')
                                                    <a href="{{ route('pengguna.shuttle-3-view-formA', $list->id) }}"><img
                                                            src="{{ asset((!empty($list->tiada_pengeluaran) && $list->tiada_pengeluaran == 1) ? 'tpbiru.png' : 'circle_check.png') }}" height='30px' alt=""
                                                            style="color: green; font-size: 20pt;" data-toggle="tooltip"
                                                            data-placement="bottom" title="{{ (!empty($list->tiada_pengeluaran) && $list->tiada_pengeluaran == 1) ? 'Borang telah disahkan PHD - Tiada Pengeluaran' : 'Borang telah disahkan PHD' }}"></i></a>
                                                @elseif ($list && $list->status == 'Dihantar ke IPJPSM')
                                                    <a href="{{ route('pengguna.shuttle-3-view-formA', $list->id) }}"><img
                                                            src="{{ asset((!empty($list->tiada_pengeluaran) && $list->tiada_pengeluaran == 1) ? 'tpbiru.png' : 'circle_check.png') }}" height='30px' alt=""
                                                            style="color: green; font-size: 20pt;" data-toggle="tooltip"
                                                            data-placement="bottom"
                                                            title="{{ (!empty($list->tiada_pengeluaran) && $list->tiada_pengeluaran == 1) ? 'Borang telah disahkan PHD - Tiada Pengeluaran' : 'Borang telah disahkan PHD' }}"></i></a>
    ...
    <script>
        function changePage() {
            ...
            window.location.href = "<?php echo URL::to('/pengguna/shuttle-4-senaraiA/" + year +"'); ?>";
        }
    </script>
